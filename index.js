...
async function updateLittleWhiteBoxExtension() {
    try {
        ...
        const message = data.isUpToDate ? '大白Y is up to date' : `大白Y updated`;
...
async function setupSettings() {
    try {
        ...
        $('#dabaiy_writing_style').val(settings.dabaiyConfig.writingStyle || '').on('input', function () {
            settings.dabaiyConfig.writingStyle = $(this).val();
            saveSettingsDebounced();
        });
        $('#dabaiy_art_style').val(settings.dabaiyConfig.artStyle || '').on('input', function () {
